...
class PolyLineTreeEnv(BaseEnvTrait):
    ...
    def step(self, action: np.ndarray):
        ...
        assert action_2d.shape[0] == self.buds_states_h.shape[0], f"action dimension is wrong, expect {self.buds_states_h.shape[0]}, got {action_2d.shape[0]}"
        self.steps += 1
        # awake bud indices (filter out sleeping nodes and not exists nodes)
        awake_bud_indices = np.array(list(filter(lambda row_idx: row_idx < self.num_bud, np.where(self.buds_states_h[:, -1] == 0)[0]))).astype(np.int32)
        num_awake_buds = awake_bud_indices.shape[0]
        if num_awake_buds > 0:
            # check and extract different actions
            # check the direction is normalized to [0,1]
            move_dis_g = utils.unscale_by_range(action_2d[awake_bud_indices, 0].reshape(-1, 1), self.delta_dis_range[0], self.delta_dis_range[1])
            ...
            branch_probs_g = utils.unscale_by_range(action_2d[awake_bud_indices, 4], self.branch_prob_range[0], self.branch_prob_range[1])
            ...
            sleep_probs_g = utils.unscale_by_range(action_2d[awake_bud_indices, 5], self.sleep_prob_range[0], self.sleep_prob_range[1])
            ...
            self.cur_rot_mats_h[awake_bud_indices] = np.matmul(delta_rot_mat, prev_rot_mat)  # multiply num_bud rotation matrixs to get new num_bud rotation matrixs
            ...
            assert len(grow_indices) == num_child_buds, f"len(grow_indices)={len(grow_indices)} does not match num_child_buds={num_child_buds}"
            ...
            if num_child_buds > 0 and self.num_bud < self.max_bud_num:
                ...
                assert np.allclose(np.linalg.norm(rot_axes, axis=1), 1), f"the length of rotate axis = {rot_axes} is not 1. get {np.linalg.norm(rot_axes)}"
                ...
                assert rot_axes.shape == (num_child_buds, 3), f"rot_axes.shape={rot_axes.shape} does not match ({num_child_buds}, 3)"
                new_branch_rot_angle = np.random.uniform(self.new_branch_rot_range[0], self.new_branch_rot_range[1], size=num_child_buds).reshape(num_child_buds, -1)
                new_branch_rot_mat = utils.rot_mat_from_axis_angles(rot_axes, new_branch_rot_angle)
                assert new_branch_rot_angle.shape == (num_child_buds, 1), f"new_branch_rot_angle.shape={new_branch_rot_angle.shape} does not match ({num_child_buds}, 1)"
                new_branch_rot_mat = self.cur_rot_mats_h[grow_indices] @ utils.rot_mat_from_axis_angles(rot_axes, new_branch_rot_angle)
                assert new_branch_rot_mat.shape == (num_child_buds, 3, 3), f"new_branch_rot_mat.shape={new_branch_rot_mat.shape} does not match ({num_child_buds}, 3, 3)"
                new_branch_euler_degree = utils.euler_from_rot_mats(new_branch_rot_mat)
                assert new_branch_euler_degree.shape == (num_child_buds, 3), f"new_branch_euler_degree.shape={new_branch_euler_degree.shape} does not match ({num_child_buds}, 3)"
        ...
        if self.steps >= self.max_grow_steps - 1 or num_awake_buds == 0:
            done = True
